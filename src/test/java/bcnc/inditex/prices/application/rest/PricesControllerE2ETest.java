...
class PricesControllerE2ETest {
    ...
    void givenARequestToGetPrices_whenApplicationDateIsNullAndProductIdIs_35455AndBrandIdIs_1_thenReturn400BadRequestAndErrorDetails() throws Exception {
        ...
        // Act
        ResultActions result = mvc.perform(MockMvcRequestBuilders.get("/prices?productId=35455&brandId=1")
                        .contentType(MediaType.APPLICATION_JSON))
                //Assert
                .andExpect(status().isBadRequest())
                .andExpect(MockMvcResultMatchers.jsonPath("$.status").value("BAD_REQUEST"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("applicationDate parameter is missing"))
                .andExpect(MockMvcResultMatchers.jsonPath("$.error").value("Required request parameter 'applicationDate' for method parameter type String is not present"));
        }
